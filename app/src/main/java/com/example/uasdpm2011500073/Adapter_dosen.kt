package com.example.uasdpm2011500073
...
class Adapter_dosen (
    ...
    private val customListItem: ArrayList<Data_campuss>
): ArrayAdapter<Data_campuss>(getContext, 0, customListItem) {
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        var listLayout = convertView
        val holder: ViewHolder
        if (listLayout == null) {
            val infLateList = (getContext as Activity).layoutInflater
            listLayout = infLateList.inflate(R.layout.activity_item_akhir, parent, false)
            holder = ViewHolder()
            with(holder) {
